...
interface Project {
  ...
  settings?: {
    businessName?: string;
    industry?: string;
    importAnalysis?: Record<string, unknown>;
...
export default function BuilderPage() {
  ...
  const projectContext = useMemo(() => {
    ...
    const analysis = (settings.importAnalysis || settings.analysisData || {}) as Record<string, unknown>;

    // Detect industry from analysis if not set
    const industry = settings.industry || detectIndustry(analysis);

    const contentSections = analysis.contentSections as Array<{ type?: string; items?: string[] }> | undefined;
    const contactInfo = analysis.contactInfo as { address?: string; phone?: string; email?: string } | undefined;
    const socialLinks = analysis.socialLinks as Array<{ platform: string }> | undefined;

    return {
      projectName: project.name,
      businessName: settings.businessName || (analysis.businessName as string),
      industry,
      location: contactInfo?.address,
      phone: contactInfo?.phone,
      email: contactInfo?.email,
      services: contentSections
      ...
      analysisData: analysis
        ? {
            ...
            issues: analysis.issues as string[] | undefined,
            strengths: analysis.strengths as string[] | undefined,
            hasBooking: contentSections?.some(
              (s) =>
                s.type?.toLowerCase().includes('book') ||
                s.type?.toLowerCase().includes('appointment')
            ),
            hasForms: contentSections?.some(
              (s) =>
                s.type?.toLowerCase().includes('form') ||
                s.type?.toLowerCase().includes('contact')
            ),
            socialLinks: socialLinks?.map((s) => s.platform),
          }
        : undefined,
    };
  }, [project]);

  // Build state change handler
  const handleBuildStateChange = useCallback((state: BuildState) => {
    setBuildState(state);
  }, []);

  // Code generated handler
...
${code}
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-dark-900">
      ...
      >
        ...
        <div className="h-16 border-b border-slate-200 dark:border-white/10 flex items-center justify-between px-6 bg-white/80 dark:bg-dark-800/80 backdrop-blur-xl shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-semibold text-lg">{project?.name || 'Project Builder'}</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              size="sm"
              className="rounded-full bg-gradient-to-r from-brand-500 to-purple-600 hover:from-brand-600 hover:to-purple-700 border-0"
            >
        ...
        <div className="flex-1 overflow-hidden p-4 bg-slate-100 dark:bg-dark-900">
          <PreviewPanel
            html={generatedCode}
            previewUrl={previewUrl}
            isLoading={buildState.isBuilding}
          />
        </div>
      </div>
      ...
      {/* AI Chat Sidebar */}
      <div
        className={`border-l flex flex-col bg-background shrink-0 transition-all duration-300 ${
          showChat ? 'w-[420px]' : 'w-0 border-l-0 opacity-0 overflow-hidden'
        }`}
      >
        {showChat && (
          <AIChat
      ...
      {!showChat && (
        <Button
          className="fixed right-4 bottom-4 shadow-lg gap-2 rounded-full"
            ...
            <div className="bg-white dark:bg-dark-800 rounded-2xl p-8 shadow-2xl border border-slate-200 dark:border-white/10">
              ...
              <div className="flex items-center justify-center gap-3 p-4 bg-brand-50 dark:bg-brand-900/20 rounded-xl">
                ...
                <span className="font-medium text-brand-700 dark:text-brand-300">
                  {buildState.currentStep || 'Initializing...'}
                </span>
              </div>
